...
func TestNewMerkleTreeFromChunks_TwoLeaves(t *testing.T) {
	chunks := [][]byte{
		[]byte("a"),
		[]byte("b"),
	}

	mt := NewMerkleTreeFromChunks(chunks)
	root := mt.GetRoot()
	if root == nil {
...
func TestNewMerkleTreeFromChunks_FullTree(t *testing.T) {
	chunks := [][]byte{
		[]byte("a"),
		[]byte("b"),
		[]byte("c"),
...
func TestNewMerkleTreeFromChunks_EmptyInput(t *testing.T) {
	var chunks [][]byte
